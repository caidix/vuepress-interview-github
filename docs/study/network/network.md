...
3. 需要防护的场景： 所有接收用户输入参数并进行数据库操作的场景。除了常见的select、insert场景，也必须警惕拼接参数使用like、havinng、group by、order by、limited、offset等子句的场景，必须进行过滤转义。
4. 防护方法：最稳妥和保险的方法只有使用预编译语句然后绑定变量，通过使用占位符，保持查询语句和数据相分离。当实在有以上like这类动态查询时才考虑白名单输入过滤、转义等方法。node可以使用Sequelize作为ORM组件，也可以使用Sequelize的绑定参数避免SQL注入。
https://github.com/mysqljs/mysql#escaping-query-values

### XSS跨站脚本
1. 原理：web页面在动态展示数据时使用了用户的输入内容，但是未做输入过滤和输出转义，导致黑客通过参数传入恶意代码，当用户浏览页面时恶意代码会被执行。
分为三种攻击形式：
- 反射型跨站脚本攻击：攻击者会通过社会工程学手段，发送一个URL连接给用户打开，在用户打开页面的同时，浏览器会执行页面中嵌入的恶意脚本。
- 存储型跨站脚本攻击：攻击者利用web应用程序提供的录入或修改数据功能，将数据存储到服务器或用户cookie中，当其他用户展示该数据的页面时，浏览器就会执行页面中嵌入的恶意脚本。所有浏览者都会收到攻击。
- DOM型跨站脚本攻击：由于HTML页面中，定义了一段JS，根据用户的输入，显示一段html代码，攻击者可以在输入时，插入一段恶意脚本，最终展示时，会执行恶意脚本。DOM跨站和以上两个跨站攻击的差别是，DOM跨站是纯页面脚本的输出，只有规范使用js，才可以防御。
2. 威胁：
- 盗取用户的cookie，伪造用户身份登陆。
- 控制用户浏览器。
- 结合浏览器及其插件漏洞，下载病毒木马到浏览者的计算机上执行。
- 衍生URL跳转漏洞。
- 让官方网站出现钓鱼网站。
- 蠕虫攻击。
3. 避免：限制用户合法输入， 增加http安全头字段： X-Content-Type-Options和X-XSS-Protection

### 点击劫持
1. 原理：攻击者在恶意站点实现了一个和信任站点及其相似的恶意页面，然后在恶意页面的上层覆盖了一个信任站点的合法页面（通常采用iframe的方式），并将合法页面设置成透明态，诱导用户点击页面，这时实际出发的是合法页面上的事件，但是恶意站点却可以劫持。
2. 防护方案： 
- 检测自己的页面是否为顶层页面，否则跳转：
```
Window !== top && (top.location.href = window.location.href)
```
- 设置http头部X-Frame-Options选项：DENY 浏览器会拒绝当前页面加载任何iframe页面；SAMEORIGIN：frame页面地址只能为同源下的页面；ALLOW-FROM origin:允许frame加载的页面地址；
- php增加安全头部： header('X-Frame-Options: SAMEORIGIN')
- node增加安全头部
```
...
### 命令注入
1. 原理：如果程序根据用户输入的参数动态生成系统命令并执行，黑客可通过传入恶意参数值注入自己定义的命令，从而控制服务器。
2. 危险：服务器被入侵控制，对内网进行渗透。
3. 防护场景： 比如system()、eval()、exec()等函数。转义或白名单处理。

### 目录穿越/任意文件读取
对./ ../ 这类没有警示，导致下载了私密文件。比如服务器内使用到了read(),readFile(),readFileSync()这类危险函数。过滤要循环过滤，切注意window、linux的路径差异化（.\和./）

### 文件上传漏洞
1. 原理：由于没有限制上传文件的类型、后缀，导致任意类型的文件可上传存储在服务器。
2. 威胁：危险地木马、病毒会存储在服务器，导致入侵检测系统报警。若用户上传HTML、SWF等网页/flash文件，可导致钓鱼工具、XSS攻击。若用户上传WebShell且可执行，可导致服务器被入侵。
3. 防护方案：
- 白名单对文件后缀进行校验
...
建议额外执行
- 使用Ceph或者对象存储放用户上传的文件，与web容器隔离
- 用户隐私相关的文件不可无访问控制上传到CDN
- 用户隐私的文件应设置权限只有属主用户才能访问到
- Nginx配置urlrewrite规则，只允许合法的Url访问

### SSRF漏洞
1. 原理：SSRF服务端请求伪造。在服务器访问页面或者HTTP服务的场景，如果接受到的目标URL是解析到内网的，则服务器会尝试访问内网。因此黑客通过提交解析到内网的URL，服务器会帮黑客对内网进行攻击、扫描。
2. 威胁：轻则出发入侵检测系统报警，重则导致内网信息泄露甚至内网入侵。
3. 需防护场景：服务器会接受外部URL或域名，并且会发起访问的场景。
3. 防护方案：提取访问目标的hostname，进行dns解析，判断ip是否为内网。并且要小心短链接指向内网或者302跳转到内网的情况，要在每次跳转都跟进目标url提取主机名解析IP，严禁外网对内网访问。

### CSRF漏洞
1. 原理：CSRF跨站请求伪造。是指劫持用户在当前已登录的web应用程序上执行非本意的操作。恶意网站通过一些技术手段（例如插入一张图片，src地址是操作请求）欺骗用户的浏览器去访问一个自己曾经认证过的网站并执行一些操作（比如财产转移、发信息等）
2. 需要防护的场景：对用户数据进行查询、操作的接口。
3. 防护方案：
- 检查Referer字段：检查HTTP请求的Referer字段是否属于本站域名，非本站域名的请求进行拒绝。
- 添加校验token：服务器生成token进行校验。
- 合理使用框架的CSRF防护功能。

### 数据脱敏
1. 问题：如果没有对敏感数据进行脱敏输出，可能会被嗅探收集、批量爬取，泄露业务核心数据。
2. 防护方案：建议把敏感的数据的写入、读取解耦成一个公共服务，对外暴露调用方法。所有对敏感数据的操作都必经这个服务，由该服务实现权限校验、自动脱敏输出。

### 逻辑漏洞
简单来说就是比如支付金额一定要大于等于0，但是没有做限制 这就是逻辑漏洞。

### XXE漏洞
1. 说明：当允许引用外部实体时，通过构造恶意内容，可导致读取任意文件，执行系统命令、探测内网端口、攻击内网网站等危害。默认情况下，大多数XML解析器都容易受到XXE攻击。
2. 防护场景：会处理接受用户提交的XML的接口
