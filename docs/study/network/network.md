...
- 布局： 这个时候我们已经构建好了响应的 DOM、CSSDOM styleSheets 树，接下来就需要计算出 DOM 树中可见元素的几何位置，这个计算的过程就叫做布局。Chrome 在布局阶段需要完成两个任务：创建布局树和布局计算。由于 DOM 树上还有许多例如 script、head 这样不需要展示的标签，也有使用了 display：none 的元素，所以在显示之前，我们需要额外的构建一棵只包含可见元素的布局树。
