...
## 28. 如何辨别代码执行在node环境还是浏览器环境
1. 内置对象不同： 浏览器提供了window全局对象， node提供global全局对象。

```js
console.log(window)
// window is not defined  那不在浏览器环境了
```
2. this的指向不同：浏览器环境全局this默认指向window， node中this默认指向空对象{}
...
3. node其实是对V8引擎进行了封装。
4. API不同，node环境没有dom和bom。
5. I/O读写。
6. 模块加载：nodejs环境提供了cmd的模块加载的api和npm包管理工具
