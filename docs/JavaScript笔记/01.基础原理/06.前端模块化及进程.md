...
浏览器加载 ES6 模块，也使用script标签，但是要加入 type="module"属性。浏览器对于带有 type="module"的script，都是异步加载，不会造成堵塞浏览器，即等到整个页面渲染完，再执行模块脚本，等同于打开了script标签的 defer 属性。
...
## 组件的按需加载
>
> 了解模块化之后，我们最常用到的场景其实就是打包组件库或是第三方包、SDK等等，以打包组件库为例。

 1. 我们需要组件库支持按需加载的能力， 需要满足：组件库以 es6 模块化方式导出
