---
title: 模块解析
date: 2020-12-17 15:01:13
permalink: /pages/b51891/
categories:
  - 《TypeScript》教程
  - 手册指南
tags: 
...
`TypeScript`是模仿`Node.js`运行时的解析策略来在编译阶段定位模块定义文件。 因此，`TypeScript`在Node解析逻辑基础上增加了`TypeScript`源文件的扩展名（ `.ts`，`.tsx`和`.d.ts`）。 同时，`TypeScript`在 `package.json`里使用字段`"types"`来表示类似`"main"`的意义 - 编译器会使用它来找到要使用的"main"定义文件。
...
> '`package.jso`n' has **'types'** field './lib/`typescript.d.ts`' that references 'node_modules/typescript/lib/`typescript.d.ts`'.
